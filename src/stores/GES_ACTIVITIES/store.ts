...
const Store = defineStore('GES_ACTIVITY', {
  state: () => {
    return {
      data: initialState,
  ...
  actions: {
    ...
    async getData() {
      if ( (this.auth?.user as any)?.id && !this.data[(this.auth.user as any).id]) {
        ...
        Api.get({
          url: '/activityFugitiveEmission/enterprise/'+(this.auth.user as any).id,
          onSuccess: (data: any[] = []) => {
            const d:any = {}
            data?.forEach((el:any)=>{
              d[el.id] = el
    ...
    async updateData({ data, callback, id , isValidation }: { data: any; callback?: () => void; id: string , isValidation?:boolean }) {
      try{
        ...
        const enterpriseId = (this.auth.user as any).id
        Api.put({
          url:'/activityFugitiveEmission/'+id,
          body:{
            ...data,
            enterpriseId , 
          ...
          onSuccess:(data:any)=>{
            ...
            console.log("return data after update", data)
    ...
    getDeclarationData({callback}:{callback:(data:any)=>void}) {
      const enterpriseId = (this.auth.user as any).id
      let url = `/declaration/filterSortAndOrder?equipmentType=ACTIVITY_OF_FUGITIVE_EMISSION`
