...
const electricalEqStore = defineStore('electricalEqStore', {
  state: () => {
    return {
      data: initialState,
  ...
  actions: {
    ...
    async getData() {
      if ( (this.auth?.user as any)?.id && !this.data[(this.auth.user as any).id]) {
        ...
        Api.get({
          url: '/electricalEquipment/enterprise/'+(this.auth.user as any).id,
          onSuccess: (data: any[] = []) => {
            const d:any = {}
            data?.forEach((el:any)=>{
              d[el.id] = el
      ...
      try{
        ...
        Api.post({
          ...
          onSuccess:(data:any)=>{
            ...
            this.ui.notifySuccess({ message: 'Équipement electrique ajouté', position: 'bottom-center' })
            this.loading = false
            callback?.()
          },
          onError:(err:any)=>{
      ...
      try{
        ...
        Api.del({
          ...
          onSuccess:()=>{
            ...
            this.ui.notifySuccess({ message: 'Équipement mobile supprimée', position: 'bottom-center' })
            this.loading = false
            callback?.()
          },
          onError:(err:any)=>{
    ...
    getDeclarationData({callback}:{callback:(data:any)=>void}) {
      const enterpriseId = (this.auth.user as any).id
      let url = `/declaration/filterSortAndOrder?equipmentType=MOBILE_EQUIPMENT`
...
export default electricalEqStore
