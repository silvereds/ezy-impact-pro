...
const Store = defineStore('coldHeatEqStore', {
  state: () => {
    return {
      data: initialState,
  ...
  actions: {
    // to get mobile equipment 
    async getData() {
      if ( (this.auth?.user as any)?.id && !this.data[(this.auth.user as any).id]) {
        this.loadingData = true
        Api.get({
          url: '/coldAndHeatEquipment/enterprise/'+(this.auth.user as any).id,
          onSuccess: (data: any[] = []) => {
            const d:any = {}
            data?.forEach((el:any)=>{
              d[el.id] = el
      ...
      try{
        ...
        Api.del({
          ...
          onSuccess:()=>{
            ...
            this.ui.notifySuccess({ message: 'Équipement froid chaleur supprimée', position: 'bottom-center' })
            this.loading = false
            callback?.()
          },
          onError:(err:any)=>{
            this.ui.notifyError("une erreur s'est produite", 'bottom-center')
            console.log("error whwn add cold heat Equipement",err)
            this.loading = false
          }
        })
      }catch(err){
    ...
    getDeclarationData({callback}:{callback:(data:any)=>void}) {
      const enterpriseId = (this.auth.user as any).id
      let url = `/declaration/filterSortAndOrder?equipmentType=COLD_AND_HEAT_EQUIPMENT`
      url = url+"&enterpriseId="+enterpriseId
      console.log("url to get declaration mobile equipment",url)
