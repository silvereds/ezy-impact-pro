...
const{data:dataStore} = storeToRefs(store)
...
<template>
    <div class="pa-2 flex flex-column gap-2 " style="width:30vw">
        ...
        <InputText v-model="data.name"  type="text" placeholder="nom du site" class="p-inputtext-sm"  />
        <InputNumber v-model="data.area" inputId="integeronly" placeholder="superficie en m2" class="p-inputtext-sm" suffix=" m2" />
        <div class="flex align-items-center justify-content-center">
            <Button label="Enregistrer" style="margin-top:1rem;width:40%" :loading="store.loading" @click="onClick()" />
        </div>
    </div>
</template>
